...
function updateTagFilterDropdown() {
  ...
  allTags.forEach(tag => {
    ...
    option.value = tag;
    option.textContent = tag === 'all' ? 'Show All' : tag;
...
function getTagOptionsHTML(selectedTag = '') {
  let options = `<option value="">No Tag</option>`;
  customTags.forEach(tag => {
...
function createNote(id, content = '', x = 100, y = 100, image = '', tag = '') {
  ...
  note.innerHTML = `
    <textarea placeholder="Click to edit">${content}</textarea>
    ${image ? `<img src="${image}" />` : ''}
    <div class="controls">
      <button class="addImage">Add Image</button>
      <button class="selectNote">Select</button>
